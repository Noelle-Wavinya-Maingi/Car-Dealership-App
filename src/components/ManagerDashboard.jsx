...
const ManagerDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [usersResponse, tasksResponse, departmentsResponse] =
        await Promise.all([fetchUsers(), fetchTasks(), fetchDepartments()]);
      ...
      const findBestDepartment = Object.keys(rates).reduce((a, b) =>
        rates[a] > rates[b] ? a : b
      );
      const bestDepartmentName =
        departmentData.find((dep) => dep.id === parseInt(findBestDepartment))
    ...
    datasets: [
      {
        ...
        data: departments.map(
          (department) => departmentCompletionRates[department.id]
        ),
        backgroundColor: departments.map(
          (_, i) => `rgba(54, 162, 235, ${0.2 + i * 0.1})`
        ), // Example gradient effect
        borderColor: departments.map(
          (_, i) => `rgba(54, 162, 235, ${1 - i * 0.1})`
        ),
        borderWidth: 1,
        hoverBackgroundColor: departments.map(
          (_, i) => `rgba(75, 192, 192, ${0.4 + i * 0.1})`
        ),
        hoverBorderColor: departments.map(
          (_, i) => `rgba(75, 192, 192, ${1 - i * 0.1})`
        ),
      },
    ],
  };
    ...
    datasets: [
      {
        ...
        data: departments.map(
          (department) => departmentIncompleteTasks[department.id]
        ),
        fill: false,
  ...
  return (
    <div className="dept-container" style={{ marginTop: "80px" }}>
      ...
      <div className="dept-card mt-4">
        <div className="dept-card-header">Incomplete Tasks By Department</div>
        <div className="dept-card-body">
      ...
      <div className="dept-card mt-4">
        <div className="dept-card-header">Best Performing Department</div>
        <div className="dept-card-body">
          <div style={{ width: "800px", height: "600px", margin: "0 auto" }}>
            <Pie data={pieChartData} options={pieChartOptions} />
          </div>
          <p className="mt-2">
            Best Performing Department: {bestPerformingDepartment}🎉🎉🎉🎉
          </p>
        </div>
      </div>
    </div>
